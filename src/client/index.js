import { createAnimationCatalog } from './animations.js';
import { createActivitySystem } from './activity.js';
import { resolveHostApi } from './host-api.js';
 ...
 *
 * 修改本目录源码后运行 npm run build，生成已提交的 lib/client.js。
 *
 * 【浏览器插件格式（重要背景）】
...
window.__ModuleLoader__.load({
    ...
    factory: (require) => {
        ...
        // 快速甩出后的惯性动画（与按住拖动的悬空反馈分开）。源视频 10 秒，
        // 甩出时直接从第 2 秒的完整球形开始，并自然播放到结尾恢复人物。
        ...
        var WALK_ANIM = TARGET_MOVE_ANIM; // 选中目标移动复用专用奔跑动画
        ...
        var INERTIA_TAU = 700; // 惯性速度衰减时间常数（ms，指数阻尼；越大阻力越小）
        var INERTIA_STOP_SPEED = 0.015; // 惯性速度低于此值（px/ms）停止
        var BOUNCE_RESTITUTION = 0.78; // 碰撞反弹：法向速度保留比例（其余为能量损耗）
        var BOUNCE_TANGENTIAL = 0.97; // 碰撞反弹：切向速度保留比例
        var BOUNCE_MIN_SPEED = 0.12; // 法向撞击低于此值（px/ms）不反弹，直接贴边吸收
        var BOUNCE_SQUASH_MIN = 0.35; // 法向撞击超过此值才播挤压动画（Q 弹反馈阈值）
        var GROUND_FRICTION_TAU = 1800; // 贴地滑行额外摩擦的时间常数（ms，滑动能量损耗）
        var COAST_STRETCH_MAX = 0.12; // 滑行中沿速度方向的最大拉伸量
        ...
        // 惯性碰撞使用人物框而非 16:9 视频舞台。返回值仍以舞台中心为坐标，
        // 方便现有位置引擎直接钳制；舞台透明留白可以自然移出视口。
        ...
        var pickFrom = (pool, exclude) => {
            ...
            var entries = (pool || []).filter((id) => {
                ...
                if (failedAnims.has(id)) {
                    if (now - failedAnims.get(id) >= ANIM_FAIL_COOLDOWN_MS)
                        failedAnims.delete(id); // 冷却过期即清理，Map 不无限增长
                    else return false;
                }
                return true;
            });
        ...
        function useVideoLayer(videoARef, videoBRef, endedRef) {
            ...
            return useCallback(
                (anim, once, onReady) => {
                    ...
                    el.src = animUrl(anim);
                    // 变球动作直接从完整球形开始，但不裁剪、不循环，播放到自然结尾。
                    // 复用双缓冲 video 播放其他动作前必须清掉专用处理器。
                    el.loop = !once;
                    el.playbackRate = 1;
                    el.onloadedmetadata =
                        anim === FLING
                            ? () => {
                                  // 等待定位到球形关键帧后再切入前景，避免短暂闪现人物站姿。
                                  el.currentTime = Math.min(FLING_START_SEC, Math.max(0, el.duration - 0.1));
                              }
                            : null;
        ...
        function createCustomStore() {
            ...
            return {
                ...
                remove(id) {
                    ...
                    return fetch('/whale-pet/api/actions/delete?name=' + encodeURIComponent(action.file), {
                        method: 'DELETE',
                    })
                        .then((r) =>
                            r.json().catch(() => ({
        ...
        function WhalePet({ config, api, store, locale, customStore, ruleRuntime, useSessions }) {
            ...
            var currentSession =
                typeof useSessions === 'function' ? useSessions((sessions) => sessions.current) : undefined;
            ...
            var fxRef = useRef(null); // 动效层（JS transform：倾斜跟随/拖拽形变）
            var breatheRef = useRef(null); // CSS 动画层（呼吸/弹跳/落定）
            ...
            useEffect(() => {
                ...
                var stop = startActivitySensor(api, (state) => arbiter.update(state.active), {
                    ...
                    onModel: (context, sessionId) => {
                        if (currentSessionRef.current && sessionId && sessionId !== currentSessionRef.current)
                            return;
            ...
            }, [api, cfg && cfg.scope, cfg?.scope === 'current' ? currentSession : undefined]);
            ...
            var startDrive = (el) => {
                ...
                // 动画时长驱动节奏，取不到时兜底
                var duration =
                    Number.isFinite(el.duration) && el.duration > 0 ? el.duration : MOVE_FALLBACK_DURATION;
            ...
            // ============================================================================
            // 拖拽惯性—— 快速甩动松手 → 动量滑行（指数阻尼 + 边界反弹）
            // ============================================================================
            // 位置由 rAF 直接写 inline style（sliding 状态让 rootStyle 不设 left/top，
            // 避免重渲染覆盖）；触到视口边界即反弹：法向速度反转并衰减（能量损耗），
            // 切向速度略损，贴地滑行另有额外摩擦；速度衰减到阈值停下，落定当前位置
            // （渲染时 clamp 保留可抓回可见条）。滑行中再按下即可抓住。
            var stopInertia = () => {
            ...
            var startInertia = (vx, vy, centerX, centerY) => {
                ...
                // 滑行结束：清滑行形变、播落定回弹、记录当前位置（反弹后恒在屏内）
                var settle = () => {
                    cur.raf = null;
                    cur.active = false;
                    setSliding(false);
                    ...
                    // 变球视频尚未播完时保持当前动画；由 ended 回调在自然结尾切回待机。
                    if (animRef.current !== FLING) playNext(IDLE, false);
                };
                // 碰撞反弹：法向速度反转并衰减（能量损耗），切向速度略损；
                // 撞击足够快才播挤压动画（Q 弹反馈），慢速贴边直接吸收不抖
                var bounceX = () => {
                    var v = Math.abs(vx);
                ...
                var step = () => {
                    ...
                    cur.cx += vx * dt;
                    cur.cy += vy * dt;
                    var decay = Math.exp(-dt / INERTIA_TAU); // 空气阻尼
                    vx *= decay;
                    vy *= decay;
                    // 视口边界反弹：人物矩形保持在屏内；视频画布的透明留白不参与碰撞
                    var W = window.innerWidth;
                    var H = window.innerHeight;
                    ...
                    // 人物脚底贴地时的额外摩擦（视频画布底边不算接地）
                    if (cur.cy >= bounds.maxY - 0.5 && vx !== 0) vx *= Math.exp(-dt / GROUND_FRICTION_TAU);
                    ...
                    if (rootEl) {
                        ...
                        rootEl.style.right = 'auto';
                        rootEl.style.bottom = 'auto';
                        livePosRef.current = { left: left, top: top };
                    }
                    fxCoast(vx, vy); // 滑行沿速度方向轻微拉伸（fx 关闭时为空操作）
            ...
            // ============================================================================
            // 交互动效：光标倾斜跟随 / 拖拽挤压拉伸 / 点击弹跳
            // ============================================================================
            // 倾斜跟随：window pointermove 只记录目标值，rAF 弹簧插值逼近，
            // 写 fxRef 的 inline transform（不触发 React 重渲染）。指针远离或
            // 值归零后循环自停。拖拽/滑行/隐藏期间暂停，形变让位给拖拽拉伸。
            var FX_TILT_RADIUS = 140; // 光标生效半径（px，自舞台边缘；超出即停止跟随）
            var FX_TILT_MAX_DEG = 3.5; // 最大倾角
            var FX_TILT_MAX_PX = 5; // 光标方向最大位移（px）
            var FX_SPRING = 0.14; // 每帧插值系数（弹簧软硬）
            var fxCfgOn = () => {
                var c = cfgRef.current;
                return !(c && c.fx === false) && !prefersReducedMotion();
            };
            var fxAnimRef = useRef({ cur: 0, tgt: 0, raf: null }); // 归一化倾向值 -1..1
            var fxRectRef = useRef({ rect: null, at: 0 }); // 舞台矩形缓存（节流刷新）
            var fxStopTilt = () => {
            ...
            var fxStepTilt = () => {
                ...
                fxApplyTilt();
                if (Math.abs(st.tgt - st.cur) > 0.001 || Math.abs(st.cur) > 0.004) {
            ...
            // 拖拽拉伸：按速度矢量沿运动方向 stretch、垂直方向压缩；
            // 直接写 transform，松手清空并由 wp-settle 动画回弹。
            var fxDragPtRef = useRef(null); // 上次拖拽采样 {x,y,t}
            var fxDragSRef = useRef(0); // 平滑后的形变量 0..0.18
            var fxDragTransform = (x, y) => {
                ...
                if (lp) {
                    ...
                    if (dt < 200) {
                        var sp = Math.hypot(x - lp.x, y - lp.y) / dt; // px/ms
                        var target = Math.min(sp * 0.09, 0.16);
                        fxDragSRef.current += (target - fxDragSRef.current) * 0.3;
                    }
                }
                fxDragPtRef.current = { x: x, y: y, t: performance.now() };
            ...
            // 点击弹跳：弹跳未结束时再次点击不重启，只加大 --pop-amp（动画连续
            // 无跳变，Q 弹感随连点递增）；已结束则恢复默认幅度重新起播
            var fxPop = () => {
                var el = breatheRef.current;
                if (!el || !fxCfgOn()) return;
                if (el.classList.contains('wp-pop')) {
                    el.style.setProperty('--pop-amp', '0.12'); // 升级：幅度加倍
                    return;
                }
                el.style.removeProperty('--pop-amp');
                el.classList.add('wp-pop'); // 播完由 onAnimationEnd 摘 class
            };
            var fxSettle = () => {
                var el = breatheRef.current;
                if (!el || !fxCfgOn()) return;
            ...
            // 碰撞挤压：撞地/天花板用纵向压扁（wp-pop），撞墙用横向压扁（wp-squash-x）
            var fxBounce = (axis) => {
                var el = breatheRef.current;
                if (!el || !fxCfgOn()) return;
                el.classList.remove('wp-pop', 'wp-squash-x', 'wp-settle');
                el.style.removeProperty('--pop-amp'); // 清掉点击弹跳升级的幅度残留
                void el.offsetWidth; // 强制 reflow 重置动画
                el.classList.add(axis === 'x' ? 'wp-squash-x' : 'wp-pop');
            };
            // 滑行拉伸：沿速度方向轻微拉长（Q 弹），落定时清空交还 wp-settle
            var fxCoast = (vx, vy) => {
                var el = fxRef.current;
                if (!el || !fxCfgOn()) return;
            ...
            // 全局指针监听：靠近宠物时轻微倾向光标；配置/reduced-motion 关闭时不注册
            useEffect(() => {
                var onMove = (e) => {
                    if (!fxCfgOn()) return;
                    if (dragRef.current.active || inertiaRef.current.active || hideRef.current.active) return;
                    var rootEl = rootRef.current;
                    if (!rootEl) return;
                    var now = performance.now();
                    ...
                    var r = c.rect;
                    var st = fxAnimRef.current;
                    // 生效区：舞台左右各 FX_TILT_RADIUS、上下留窄边；区外目标归零
                    // （循环自然衰减后自停）。此前横向只钳不归零，导致全屏都在跟随
                    if (
                        e.clientY < r.top - 40 ||
            ...
            var handlePointerDown = (e) => {
                ...
                fxStopTilt(); // 暂停倾斜跟随（拖拽期间由挤压拉伸接管 fx 层）
                fxAnimRef.current.tgt = 0;
                ...
                if (inertiaRef.current.active) {
                    ...
                    setSliding(false);
                    if (last)
                        setCustomPos({ rx: last.cx / window.innerWidth, ry: last.cy / window.innerHeight });
                }
            ...
            var handlePointerMove = (e) => {
                ...
                // 拖拽挤压拉伸（fx 关闭/reduced-motion 时内部为空操作，直接清形变）
            ...
            var handlePointerUp = (e) => {
                ...
                if (wasDragging) {
                    ...
                    // 隐藏中松手不进惯性：反弹会把屏外的宠物钳回屏幕（保持"拖出即藏"语义）
                    if (!hideRef.current.active && v && v.speed > INERTIA_MIN_SPEED && !reduced) {
                        setSliding(true);
                        playNext(FLING, true); // 从球形段起播，并在物理停止后继续播放到结尾
            ...
            var handleClick = (e) => {
                ...
                if (now - lastClickAtRef.current < DOUBLE_CLICK_MS) {
                    ...
                    setSelected(false);
                    // 双击不另起弹跳：首次单击的 pop 尚在播放，鲸鱼现世自带冲击反馈
            ...
            var menuOpenSettings = () => {
                ...
                var settingsButton =
                    document.querySelector('.hHd-Xa_settingsArea button') || findNamed(['设置', 'Settings']);
                if (!settingsButton) {
                    ...
                    : hide.active
                      ? (() => {
                            ...
                            if (hide.edge === 'left') {
                                ...
                                s.top =
                                    (peek ? window.innerHeight - keepVisibleY : window.innerHeight) + 'px';
                                s.left = (peek ? clampLeft(px - halfW) : baseLeft) + 'px';
                            }
                            return s;
                        })()
            ...
            // 渲染树：root > stage > [fx > breathe > [video A, video B], 命中层]
            // fx 层承载 JS 指令式形变（倾斜/拖拽拉伸），breathe 层承载 CSS 动画
            // （呼吸/弹跳/落定）；命中层在最上层承载交互，几何不受形变影响
            return h('div', {
                ref: rootRef,
        ...
        var LOCALES = {
            ...
            en: {
                ...
                uploadCustomUnavailable: 'The upload service is not loaded; restart Harness and try again',
                deleteCustom: 'Delete',
                                    ...
                                    children: [
                                        ...
                                        h(
                                            'button',
                                            {
                                                ...
                                                'aria-label': t('removeAction') + localizedAnimName(id, t),
        ...
        function RuleEditor({ rule, isNew, options, onSave, onCancel, onTest, t }) {
            ...
            var [cooldownSec, setCooldownSec] = useState(
                Math.max(1, Math.round((rule.cooldownMs || 30000) / 1000))
            );
                ...
                patch(() =>
                    setConds((cs) => {
                        var next = cs.filter(
                            (c) =>
                                c.field !== 'type' &&
                                (value === 'tool/call' || value === 'tool/result' || c.field !== 'toolName')
                        );
            ...
            var setSimpleTool = (value) =>
                patch(() =>
                    setConds((cs) => {
                        ...
                        return value.trim()
                            ? next.concat([{ field: 'toolName', op: 'contains', value: value }])
                            : next;
                    })
                );
            // 内联 label 子元素：静态数组也统一带 key（消除 React 列表告警）
            var inline = (key0, label, control, hint, unit) =>
                h('label', { className: 'wp-sf-inline', key: key0, title: hint || undefined }, [
                    label,
                    h('span', {
                        key: key0 + '-c',
            ...
            var fieldSelect = (c, i) => {
                ...
                return h(
                    'select',
                    {
                        ...
                        onChange: (e) =>
                            setCond(i, { field: e.target.value === '__custom__' ? 'data.' : e.target.value }),
                    },
                                        ...
                                        h(
                                            'button',
                                            {
                                                ...
                                                onClick: () =>
                                                    patch(() => {
                                                        setConds([
                                                            { field: 'type', op: 'eq', value: 'turn/end' },
                                                        ]);
                                                        setAdvanced(true);
                                                    }),
                                            },
            ...
            return h('div', {
                ...
                children: [
                                    ...
                                    h('div', {
                                        ...
                                        children: [
                                            h(
                                                ...
                                                EVENT_TYPES.map((et) =>
                                                    h('option', { key: et, value: et }, eventLabelOf(et, t))
                                                )
                                            ),
                    ...
                    !actions.length &&
                        h('p', { className: 'wp-sf-customHint wp-rule-missing' }, t('ruleMissingActions')),
                    h(
                        'button',
                        {
                            type: 'button',
                                    ...
                                    h(
                                        'button',
                                        {
                                            ...
                                            onClick: () =>
                                                patch(() =>
                                                    setConds((cs) =>
                                                        cs.concat([{ field: 'type', op: 'eq', value: '' }])
                                                    )
                                                ),
                                        },
                                ...
                                h('label', {
                                    ...
                                    children: [
                                        ...
                                        h('input', {
                                            ...
                                            value: priority,
                                            onChange: (e) =>
                                                patch(() => setPriority(parseInt(e.target.value, 10) || 0)),
                                        }),
                    ...
                    h('div', {
                        ...
                        children: [
                            h(
                                'button',
                                { type: 'button', className: 'wp-rule-btn', onClick: () => onTest(build()) },
                                    ...
                                    h('div', {
                                        ...
                                        children: [
                                            h(
                                                'span',
                                                null,
                                                Math.round((r.cooldownMs || 30000) / 1000) +
                                                    ' ' +
                                                    t('repeatEvery')
                                            ),
                                            h(
        ...
        function SettingsForm({
            ...
            // 系统"减少动态效果"偏好（响应式）：开启时交互动效被运行时强制关闭，
            // 开关需如实反映生效状态（未勾选 + 禁用）；OS 偏好变化时实时同步 UI
            ...
            useEffect(() => {
                ...
                else mq.addListener(onChange); // 旧实现兜底
                return () => {
                ...
                h('div', {
                    ...
                    children: [
                        h('div', {
                            ...
                            title: tooltipText || hintText || undefined,
                            children: [
                                labelText,
            ...
            var activityLevel = Object.keys(activityPresets).reduce((best, key) => {
                var score = ['idleProb', 'turnProb', 'actProb', 'moveProb'].reduce(
                    (sum, field) => sum + Math.pow((behavior[field] || 0) - activityPresets[key][field], 2),
                    0
                );
            ...
            var paramsContent = h('div', {
                children: [
                    h('div', {
                        ...
                        children: [
                            toggleItem('visible', t('labelVisible'), t('hintVisible'), !!draft.visible, (e) =>
                                onPatch((d) => ({ ...d, visible: e.target.checked }))
                            ),
                            toggleItem('bubbles', t('labelBubbles'), t('hintBubbles'), !!draft.bubbles, (e) =>
                                onPatch((d) => ({ ...d, bubbles: e.target.checked }))
                            ),
                                ...
                                children: [
                                    h(
                                        'span',
                                        {
                                            ...
                                            title: (custom && custom.display) || undefined,
                                        },
