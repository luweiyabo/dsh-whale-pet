...
test('允许浏览器同源请求', () => {
    assert.equal(
        isAllowedApiRequest(
            req({ 'sec-fetch-site': 'same-origin', host: 'localhost:8123', origin: 'http://localhost:8123' })
        ),
        true
    );
    assert.equal(
        isAllowedApiRequest(
            req({ 'sec-fetch-site': 'same-origin', host: '127.0.0.1:8123', origin: 'http://127.0.0.1:8123' })
        ),
        true
    );
    // 地址栏直接打开（顶层导航）
