...
test('解析 Kimi 可用余额且不估算 Token 成本', () => {
  assert.deepEqual(parseProviderBalances('moonshot', {
    status: true,
    data: { available_balance: 49.58894, voucher_balance: 46.58893, cash_balance: 3.00001 },
  }), [{ currency: 'CNY', total: '49.58894' }]);
  assert.deepEqual(parseProviderBalances('moonshot', { status: false, data: { available_balance: 10 } }), []);
});

test('解析阶跃星辰可用余额并拒绝格式错误的值', () => {
  assert.deepEqual(parseProviderBalances('stepfun', {
    object: 'account',
    type: 'prepaid',
    balance: 26.00,
