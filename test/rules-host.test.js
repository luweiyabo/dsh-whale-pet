...
test('保存时拒绝过长的规则字符串', () => {
    const schema = buildSettingsSchema(z);
    assert.throws(() =>
        schema({ rules: [{ id: 'x'.repeat(65), when: [{ field: 'type', op: 'eq', value: 'y' }] }] })
    );
    assert.throws(() =>
        schema({
            rules: [{ id: 'x', name: 'n'.repeat(65), when: [{ field: 'type', op: 'eq', value: 'y' }] }],
        })
    );
    assert.throws(() =>
        schema({ rules: [{ id: 'x', when: [{ field: 'f'.repeat(129), op: 'eq', value: 'y' }] }] })
    );
    assert.throws(() =>
        schema({ rules: [{ id: 'x', when: [{ field: 'type', op: 'eq', value: 'v'.repeat(257) }] }] })
    );
    assert.throws(() =>
        schema({
            rules: [{ id: 'x', bubble: 'b'.repeat(51), when: [{ field: 'type', op: 'eq', value: 'y' }] }],
        })
    );
});
...
test('自定义动作上传仅接受安全的 WebM 或 MP4 文件名并使用自定义分类', () => {
    assert.deepEqual(validateCustomActionFileName('my-dance.webm'), {
        id: 'my-dance',
        ext: 'webm',
        file: 'my-dance.webm',
        category: 'custom',
    });
    assert.deepEqual(validateCustomActionFileName('鲸鱼挥手.MP4'), {
        id: '鲸鱼挥手',
        ext: 'mp4',
        file: '鲸鱼挥手.mp4',
        category: 'custom',
