...
const {
  INTENT, INTENT_ACTIONS, INTENT_CFG_KEY,
  emptyIntentState, applyFrame, highestIntent,
  mergeConfig, startActivitySensor, createArbiter,
  parseSessionModelSelection,
  pickFrom, setCustomAnims,
  ANIM_FAIL_COOLDOWN_MS, failedAnims,
} = plugin.__internals;
...
const BEHAVIOR_MATRIX = [
  ...
  ['Agent 运行中（兜底 WORKING）', [{ type: 'host/session-status', running: true }], INTENT.WORKING, ['eating_tokens']],
  ...
  ['等待审批 approval/requested', [{ type: 'approval/requested' }], INTENT.WAITING_USER, ['looking_around']],
  ['等待回答问题 question/requested', [{ type: 'question/requested' }], INTENT.WAITING_USER, ['looking_around']],
  ['收到用户消息 user/message', [userMessage()], INTENT.LISTENING, ['attentive_listening']],
  ['用户消息排队 session/queue', [{ type: 'session/queue', items: [{ id: 1 }] }], INTENT.LISTENING, ['attentive_listening']],
  ['工具执行失败 tool/result.error', [toolCall('bash'), toolResult('t1', 1, 'boom')], INTENT.ERROR, ['jump_scare']],
  ['Agent 出错 host/agent-error', [{ type: 'host/agent-error' }], INTENT.ERROR, ['jump_scare']],
  ['回合结束 turn/end（全部工作意图清空）', [toolCall('bash'), { type: 'session/event', event: { type: 'turn/end', data: {} } }], INTENT.IDLE, []],
];
...
test('感知→仲裁全链路：写代码→等待→倾听→思考→回合结束', async () => {
  ...
  const api = {
    events: {
      async *host(_input, signal) { await waitForAbort(signal); },
      async *mux(_input, signal) {
        for (const f of frames) { yield f; await sleep(1); }
...
test('解析 session.models 响应得到当前会话模型选择', () => {
  ...
  assert.deepEqual(
    parseSessionModelSelection({ current: { provider: 'moonshot', model: 'kimi-k2', reasoningEffort: 'high' } }),
