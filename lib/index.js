...
export function buildSettingsSchema(zz = z) {
	return zz.object({
		...
		position: zz.union([zz.const('bottom-right'), zz.const('bottom-left'), zz.const('top-right'), zz.const('top-left')]).default('bottom-right'),
		scope: zz.union([zz.const('current'), zz.const('any')]).default('current'), // 感知范围
		behavior: zz.object({
			idleProb: zz.number().min(0).max(1).default(0.30),
			turnProb: zz.number().min(0).max(1).default(0.10),
			actProb: zz.number().min(0).max(1).default(0.40),
			moveProb: zz.number().min(0).max(1).default(0.20),
...
export async function scanCustomActions(dir) {
  ...
  for (const ent of entries) {
    ...
    if (!m) continue;
    const id = m[1];
    if (!id || id === '.' || id === '..' || id.includes('/') || id.includes('\\') || id.includes('\0')) continue;
    let size = 0;
    let mtime = 0;
    try {
...
export function validateCustomActionFileName(name) {
  ...
  for (let i = 0; i < name.length; i += 1) {
    ...
    if (code < 32 || code === 47 || code === 92
      || code === 34 || code === 42 || code === 58
      || code === 60 || code === 62 || code === 63 || code === 124) return undefined;
  }
...
function apply(ctx, config) {
  ...
  ctx.settings.register(SETTINGS_NS, buildSettingsSchema(z), {
    applies: 'live',
    base: config && typeof config.size === 'number'
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      // 相比 existsSync→stat 两步还消除了 TOCTOU 竞态窗口。
      let st;
      try {
        st = await lstat(file);
      } catch {
      ...
      res.writeHead(200, {
        ...
        'cache-control': isCustom
          ? (versionedCustom ? 'public, max-age=31536000, immutable' : 'no-store')
          : 'public, max-age=3600',
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      if (Number.isFinite(declared) && (declared <= 0 || declared > MAX_ACTION_UPLOAD_BYTES)) {
        send(413, { ok: false, error: 'invalid-file-size', maxBytes: MAX_ACTION_UPLOAD_BYTES });
        return;
      }
      ...
      if (availableBytes <= 0) {
        send(413, { ok: false, error: 'storage-quota-exceeded', maxStorageBytes: MAX_ACTION_STORAGE_BYTES });
        return;
      }
      // 为本次请求预留其最多可写容量；并发上传不能共同越过总配额。
      const uploadBudget = Math.min(MAX_ACTION_UPLOAD_BYTES, availableBytes);
      ...
      try {
        ...
        send(tooLarge ? 413 : 400, {
          ok: false,
          error: tooLarge && uploadBudget < MAX_ACTION_UPLOAD_BYTES ? 'storage-quota-exceeded'
            : (tooLarge ? 'invalid-file-size' : 'upload-failed'),
          ...(tooLarge && uploadBudget < MAX_ACTION_UPLOAD_BYTES ? { maxStorageBytes: MAX_ACTION_STORAGE_BYTES } : {}),
        });
      } finally {
        reservedUploadBytes -= uploadBudget;
      }
  ...
  // Custom action deletion: safe filename, same-origin request, regular files only.
  ctx.effect(() => ctx.webServer.register({
    kind: 'exact',
    ...
    handler: async (req, res) => {
      ...
      let st;
      try { st = await lstat(target); } catch { st = null; }
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        if (provider.unsupported) {
          send({ ok: false, unsupported: true, route, model, error: '当前模型服务商不支持官方余额查询' });
          return;
        }
        ...
        if (!key) {
          send({ ok: false, provider: provider.id, route, model, error: provider.label + ' API key 未配置' });
          return;
        }
        ...
        const upstreamError = body && typeof body === 'object' ? (body.error || body.message) : undefined;
        const data = {
          ok: r.ok && balances.length > 0,
          provider: provider.id,
          providerLabel: provider.label,
          balances,
          // Keep the original single-balance field for older clients.
          balance: balances[0] || null,
          error: upstreamError
            ? String(upstreamError.message || upstreamError)
            : (r.ok && balances.length === 0 ? provider.label + ' 余额响应格式无效' : undefined),
        };
